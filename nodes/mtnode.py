...
class XSensDriver(object):
    ...
    def convert_quat(self, q, source, dest=None):
        ...
        if source == 'ENU':
            if dest == 'ENU':
                ...
                return quaternion_multiplication(q_enu_ned, q)
            elif dest == 'NWU':
                return quaternion_multiplication(q_enu_nwu, q)
        elif source == 'NED':
            if dest == 'ENU':
                return quaternion_multiplication(q_ned_enu, q)
            elif dest == 'NED':
                return q
            elif dest == 'NWU':
                return quaternion_multiplication(q_ned_nwu, q)
        elif source == 'NWU':
            if dest == 'ENU':
                return quaternion_multiplication(q_nwu_enu, q)
            elif dest == 'NED':
                return quaternion_multiplication(q_nwu_ned, q)
            elif dest == 'NWU':
                return q
    ...
    def fill_from_Orient(self, orient_data):
        ...
        w, x, y, z = self.convert_quat((w, x, y, z), orient_data['frame'])
    ...
    def fill_from_Auxiliary(self, aux_data):
        ...
        try:
            self.anin1_msg.data = aux_data['Ain_1']
            self.pub_anin1 = True
        except KeyError:
            pass
        try:
            self.anin2_msg.data = aux_data['Ain_2']
            self.pub_anin2 = True
        except KeyError:
            pass
    ...
    def fill_from_Timestamp(self, o):
        ...
        try:
            ...
            (y, m, d, hr, mi, s, ns, f) = (
                o['Year'], o['Month'], o['Day'], o['Hour'], o['Minute'], o['Second'], o['ns'], o['Flags'])
            if f & 0x4:
        ...
        try:
            itow = o['TimeOfWeek']
            secs, nsecs = self.stamp_from_itow(itow)
    ...
    def fill_from_GNSS(self, o):
        ...
        try:  # PVT
            # time block
            itow, y, m, d, ns, f = (
                o['itow'], o['year'], o['month'], ['day'], o['nano'], o['valid'])
            if f & 0x4:
                secs, nsecs = self.stamp_from_itow(itow, y, m, d, ns)
    ...
    def fill_from_GPS(self, o):
        ...
        try:    # SOL
            ...
            if (f & 0x0C) == 0xC:
                secs, nsecs = self.stamp_from_itow(itow, ns=ns, week=week)
        ...
        try:    # Time UTC
            itow, y, m, d, ns, f = (
                o['iTOW'], o['year'], o['month'], o['day'], o['nano'], o['valid'])
            if f & 0x4:
                secs, nsecs = self.stamp_from_itow(itow, y, m, d, ns)
    ...
    def fill_from_Velocity(self, o):
        ...
        x, y, z = self.convert_coords(o['velX'], o['velY'], o['velZ'],
                                      o['frame'])
        self.vel_msg.twist.linear.x = x
        self.vel_msg.twist.linear.y = y
        self.vel_msg.twist.linear.z = z
    ...
    def get_handler_by_name(self, name):
        return getattr(self, "fill_from_%s" % (name.replace(" ", "_")))
